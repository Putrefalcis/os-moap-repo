...
#define command command_must_not_be_used
 ...
 again:
		...
		vi_case('\r'|VI_CMDMODE_BIT:)
			...
			break;
		case CTRL('C'):
			/* OS-MOAP patch: on WASI there is no kernel tty to turn ^C into
			 * SIGINT, so the byte arrives as data. Behave like a real shell:
			 * show ^C, discard the line, return empty for a fresh prompt. */
			printf("^C");
			goto_new_line();
			command_len = 0;
			command_ps[0] = BB_NUL;
